...
export interface AbsentCardProps { }


export const AbsentCard = ({ }: AbsentCardProps) => {
    ...
    const onSubmit = () => {
        ...
        setLoading(true);
        //console.log(values);
        appointments.update(record.id, { remarks: "", status: "ABSENT" })
            .then(updated => {
                updater(updated);
                toast({
                    variant: "default",
    ...
    return <Card className={"card"}>
            <CardHeader>
                <div className="font-semibold">Marcar como ausente</div>
            </CardHeader>
            <CardContent>
                { !editMode && <p>Usá esta opción sólo si el paciente se ausentó al turno.</p> }
                { editMode && <p>
                    ¿Estás seguro?
                </p>}
            </CardContent>
                {canEdit && <div className={"flex flex-row justify-end p-3 gap-3 pt-0"}>
                    <Button disabled={loading} type={"button"} variant={"ghost"} onClick={() => setEditMode(!editMode)}>
                        ...
                        {!editMode && <><OctagonX className={"pr-2"}/>Marcar como ausente</>}</Button>
                    {
                        (editMode)
                        && <Button onClick={onSubmit}  variant={"destructive"} disabled={loading}>
                            {loading && <Spinner className={"w-4 h-4 mr-2"}/>}
                            {loading && "Guardando"}
                            {!loading && "Cerrar turno"}
                        </Button>}
                </div>}
            </Card>;
};
