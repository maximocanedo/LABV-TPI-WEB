...
import {LocalPatientContextMenu} from "./PatientContextMenu";
    ...
    if (viewMode == ViewMode.LITTLE_CARDS) {
        return <div className=" border rounded-lg pr-5 overflow-hidden ">
            <TableRow selectable={false} className={"w-full" + (className ?? "")}>
                <TableCell
                    className={"w-[60px] text-center px-[18px] items-center justify-center rounded bg-muted"}><UserRound/></TableCell>
                <TableCell className={"pl-4 grid" + (gridColsClass)}>

                    <LocalPatientContextMenu patient={record}><div
    ...
    return (
        <TableRow selectable={false} className={"w-full " + (className ?? "") + " "}>
            ...
            {viewMode === ViewMode.TABLE && <TableCell>
                ...
                <LocalPatientContextMenu patient={record}>
